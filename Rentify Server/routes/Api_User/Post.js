...
router.get("/list/:user_id", async (req, res) => {
  ...
  try {
    ...
    if (!posts.length) {
      return res
        .status(404)
        .json({ message: "Không có bài đăng nào của người dùng này." });
    }
...
router.get("/list/type/:post_type", async (req, res) => {
  ...
  try {
    ...
    if (!posts.length) {
      return res
        .status(404)
        .json({ message: "Không có bài đăng nào thuộc loại này." });
    }
...
router.get("/detail/:id", async (req, res) => {
  try {
    ...
    const buildingPostCount = await Post.countDocuments({
      building_id: post.building_id._id,
    });
...
router.get("/buildings", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy tòa nhà", error: error.message });
  }
});
...
router.get("/rooms", async (req, res) => {
  try {
    ...
    if (!rooms.length) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy phòng trong tòa nhà" });
    }
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy phòng", error: error.message });
  }
});
    ...
    try {
      ...
      const newPost = new Post({
        ...
        building_id: building_id
          ? new mongoose.Types.ObjectId(building_id)
          : null, // Kiểm tra nếu có building_id
        ...
        photo: req.files["photo"]
          ? req.files["photo"].map((file) => file.path.replace("public/", ""))
          : [],
      ...
      res
        .status(400)
        .json({ message: "Lỗi thêm bài đăng", error: error.message });
    }
  }
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      existingPost.building_id = building_id
        ? new mongoose.Types.ObjectId(building_id)
        : existingPost.building_id;
      existingPost.room_id = room_id
        ? new mongoose.Types.ObjectId(room_id)
        : existingPost.room_id;
      ...
      if (req.files["video"]) {
        existingPost.video = req.files["video"].map((file) =>
          file.path.replace("public/", "")
        );
      }
      if (req.files["photo"]) {
        existingPost.photo = req.files["photo"].map((file) =>
          file.path.replace("public/", "")
        );
      }
...
router.get("/search", async (req, res) => {
  try {
    ...
    if (!query) {
      return res
        .status(400)
        .json({ message: "Từ khóa tìm kiếm không được cung cấp" });
    }
